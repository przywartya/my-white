...
function getTopMenu() {
    return (0, _hyperhtml2.default)(_templateObject);
}
...
function getImagesForCategory(category) {
    var imgCount = categoriesRegistry[category];
    if (imgCount && imgCount > 0) {
        var finalHtml = [];
        for (var i = 1; i <= imgCount; i++) {
            var name = categoryNamesMap[category];
            var url = 'assets/' + category + '/' + i + '.jpg';
            var description = 'Marlena Michalska, projektowanie wn\u0119trz, metamorfoza ' + name + '.';
            var imgString = (0, _hyperhtml2.default)(_templateObject2, url, description);
